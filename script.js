...
function drawWheel() {
  ...
  if (num === 0) {
    ...
    ctx.font = `bold ${canvas.width / 12}px Montserrat`;
  ...
  for (let i = 0; i < num; i++) {
    ...
    ctx.fillStyle = i % 2 === 0 ? "#751a42" : "#51032c";
    ctx.fill();
...
function finishSpin(finalRotation) {
  ...
  descriptionEl.textContent = selected.desc; // CORREGIDO: sin repetir "Definición:"
  factEl.textContent = selected.fact;       // CORREGIDO: sin repetir "Dato curioso:"
